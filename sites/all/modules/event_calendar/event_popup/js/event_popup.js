...
(function ($) {
  Drupal.behaviors.event_popup = {
    attach: function(context, settings) {
      ...
      if (settings.event_popup.classes) {
        ...
        if (!event.metaKey && !$(this).hasClass('simpleEventProcessed')) {
          ...
          $(this).click(function(event) {
            ...
            /* Custom code on 05/04/2018 Start*/
            var nodeId = $(this).attr('href').split("/")[3];//alert(nodeId);
            var locURL= document.location.href.split("event-created")[0];//alert(locURL);
            url=locURL+"capacity/get-event-details/"+nodeId;//alert(url);
            /* Custom code on 05/04/2018 End*/

            // Use defaults if not provided
            var selector = $(this).attr('name') ? 'event-calendar' : 'event_calendar';

            var options =  Drupal.event_popup.explodeOptions('width:auto;height:auto;position:[350,400]');
      ...
      if(op) {
        $('table.full tr td, table.mini tr td', context).click(function () { return false;
